//! OSS deviation: integration tests for our own AGPL AWS Secrets Manager secret
//! backend (compiled under the `aws_secrets_manager` feature). The EE variant is
//! covered by `aws_sm_integration.rs` (gated on `private`/`enterprise`); this file
//! exercises the OSS impl without touching that upstream test file.
//!
...
//!
//!    ```bash
//!    RUN_AWS_SM_TESTS=1 SQLX_OFFLINE=true cargo test -p windmill-common \
//!      --features aws_secrets_manager --test aws_sm_oss_integration -- --nocapture
//!    ```
//!
...
mod tests {
    ...
    macro_rules! skip_if_no_aws_sm {
        () => {
            if !should_run_aws_sm_tests() {
                ...
                println!("To run: RUN_AWS_SM_TESTS=1 SQLX_OFFLINE=true cargo test -p windmill-common --features aws_secrets_manager --test aws_sm_oss_integration -- --nocapture");
    ...
    fn test_settings() -> AwsSecretsManagerSettings {
        AwsSecretsManagerSettings {
            ...
            prefix: Some("windmill-oss-test/".to_string()),
        }
    }
